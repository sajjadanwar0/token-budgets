...
#   - Coq 8.18 (apt install coq OR opam) — only if --formal-only or default
#       * Tier A (pure) needs only the Coq stdlib.
#       * Tier B (Iris) needs coq-iris + coq-stdpp in the SAME switch as coqc
#         (opam install coq-iris coq-stdpp, OR apt install libcoq-iris libcoq-stdpp).
#       * Tier C (RustBelt) additionally needs a COMPILED lambda-rust checkout
#         built by the SAME coqc; point LRUST_BASE at it (default ~/lambda-rust).
...
# lambda-rust checkout used by the Coq Tier C build (override by exporting it).
LRUST_BASE="${LRUST_BASE:-$HOME/lambda-rust}"
...
CIRR_A="${CLUSTER_RATER_A:-$CIRR_DIR/cluster_irr_rater_a_frozen.csv}"
CIRR_B="${CLUSTER_RATER_B:-$CIRR_DIR/cluster_irr_rater_b_frozen.csv}"
...
if command -v coqc >/dev/null; then
    ...
    log "  coqc: $(command -v coqc)"
    log "  Coq:  $(coqc --version 2>/dev/null | head -1)"

    rm -f Makefile.conf .Makefile.d Makefile.coq Makefile.coq.conf 2>/dev/null || true
    if [ -f GNUmakefile ] && grep -q "^tier-b:" GNUmakefile 2>/dev/null; then
        : # hand-written GNUmakefile present; make will use it
    elif [ -f Makefile ] && grep -q "^tier-b:" Makefile 2>/dev/null \
         && ! head -2 Makefile 2>/dev/null | grep -qi "generated by coq_makefile"; then
        : # hand-written Makefile present and not a coq_makefile output
    else
        log "  WARN: no hand-written build file with tier targets found."
        log "        A coq_makefile-generated Makefile (no lambda-rust paths, no tier"
        log "        targets) will be used and Tier C will fail. Commit the hand-written"
        log "        GNUmakefile to token-budgets-formals/coq (the repo gitignores Makefile)."
        log "        See token-budgets-formals/coq/README.md."
    fi

    make clean >/dev/null 2>&1 || true

    # The Coq phase is BEST-EFFORT and does NOT gate the reproduction: the
    # RustBelt (Tier C) layer mechanises the OPEN Conjecture 1 and is not
    # asserted complete in the paper, and the closed Tiers A/B need coq-iris /
    # coq-stdpp present in the active switch. On any failure we log a SKIP (no
    # FAIL_COUNT increment), matching the original script's semantics.
    COQ_FULL=0
    if [ -f "$LRUST_BASE/lifetime/lifetime.vo" ]; then
        log "  lambda-rust found at $LRUST_BASE — building all tiers (A/B/C + assumptions audit)"
        if make all LRUST_BASE="$LRUST_BASE" >/tmp/_coq.log 2>&1; then
            ok "Coq: full build succeeded (Tiers A/B/C); VerifyAssumptions closed under the global context"
            COQ_FULL=1
        fi
    else
        log "  lambda-rust .vo not found under LRUST_BASE=$LRUST_BASE — Tier C (RustBelt) will be skipped"
        log "  (set LRUST_BASE=/path/to/built/lambda-rust for the full Tier C build)"
    fi

    if [ "$COQ_FULL" -eq 0 ]; then
        # Either no lambda-rust, or the full build did not complete. Fall back to
        # the closed pure + Iris tiers, which are the paper's asserted-complete part.
        if make tier-b >/tmp/_coq_tierb.log 2>&1; then
            ok "Coq: Tiers A + B compiled (pure + Iris, zero axioms); Tier C (RustBelt) skipped — open Conjecture 1"
        else
            log "  SKIP: Coq build did not complete (see /tmp/_coq.log and /tmp/_coq_tierb.log)."
            log "        This layer is best-effort and does NOT fail the reproduction. Common causes:"
            log "          - coq-iris / coq-stdpp not installed in the active switch (Tiers A/B);"
            log "          - lambda-rust not built by the SAME coqc as this project (Tier C):"
            log "            'inconsistent assumptions over Coq.Init.Ltac' -> rebuild lambda-rust"
            log "            under this switch, then make clean && make all;"
            log "          - the cloned token-budgets-formals predates the coq/ fixes (push them)."
            log "        See token-budgets-formals/coq/README.md (golden rules + troubleshooting)."
        fi
    fi
    cd "$ROOT"
else
    log "  (coqc not found; Coq verification skipped)"
fi
